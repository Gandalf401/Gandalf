...
from src.configuration.support.torch_expansion.depthwise_conv2d import DepthwiseConv2d as torch_conv2d
from src.configuration.support.jittor_expansion.depthwise_conv2d import DepthwiseConv2d as jittor_conv2d
...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if dilation[0] != 1 and stride[0] != 1 or dilation[1] != 1 and stride[1] != 1:
        raise Exception('Dilation and stride of DepthwiseConv2d cannot be unequal to 1 at the same time.')
    ...
    if framework == 'TensorFlow':
        ...
        return eval('tf.keras.layers.DepthwiseConv2D' + params_str), out_shape
        ...
        if padding == 'valid':
            ...
            if pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(in_channels={0}, kernel_size={1}, stride={2}, padding=({3}, {4}), dilation={5}, ' \
                             'bias={6}, depth_multiplier={7})'.format(in_channels, kernel_size, stride,
                                                pad_h // 2, pad_w // 2, dilation, bias, depth_multiplier)
                return eval('torch_conv2d' + params_str), out_shape
            else:
