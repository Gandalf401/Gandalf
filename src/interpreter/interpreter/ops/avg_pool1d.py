...
from src.configuration.support.jittor_expansion.avg_pool1d import AvgPool1d
...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if framework == 'TensorFlow':
        return tf.keras.layers.AveragePooling1D(pool_size, stride, padding), out_shape
    if shape_only:
        return out_shape
    elif framework == 'PyTorch':
        if padding == 'valid':
            params_str = '(kernel_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('torch.nn.AvgPool1d' + params_str), out_shape
        else:
            ...
            if pad % 2 == 0:
                ...
                params_str = '(kernel_size={0}, stride={1}, padding={2})'.format(pool_size, stride, pad // 2)
                return [pad_layer, eval('torch.nn.AvgPool1d' + params_str)], out_shape
    elif framework == 'Jittor':
        if padding == 'valid':
            params_str = '(pool_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('AvgPool1d' + params_str), out_shape
        else:
            pad = cal_same_padding_1d(input_shape, pool_size, stride)
            if pad % 2 == 0:
                ...
                params_str = '(pool_size={0}, stride={1}, padding={2})'.format(pool_size, stride, pad // 2)
                return [pad_layer, eval('AvgPool1d' + params_str)], out_shape
    else:
