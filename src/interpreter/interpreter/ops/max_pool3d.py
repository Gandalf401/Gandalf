...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if framework == 'TensorFlow':
        return tf.keras.layers.MaxPooling3D(pool_size, stride, padding), out_shape
    elif framework == 'PyTorch':
        if padding == 'valid':
            params_str = '(kernel_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('torch.nn.MaxPool3d' + params_str), out_shape
        else:
            ...
            if pad_d % 2 == 0 and pad_h % 2 == 0 and pad_w % 2 == 0:
                ...
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}, {4}))' \
                    .format(pool_size, stride, pad_d // 2, pad_h // 2, pad_w // 2)
                return [pad, eval('torch.nn.MaxPool3d' + params_str)], out_shape
    elif framework == 'Jittor':
        if padding == 'valid':
            params_str = '(kernel_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('jittor.nn.MaxPool3d' + params_str), out_shape
        else:
            pad_d, pad_h, pad_w = cal_same_padding_3d(input_shape, pool_size, stride)
            if pad_d % 2 == 0 and pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}, {4}))' \
                ...
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}, {4}))' \
                    .format(pool_size, stride, pad_d // 2, pad_h // 2, pad_w // 2)
                return [pad, eval('jittor.nn.MaxPool3d' + params_str)], out_shape
    else:
