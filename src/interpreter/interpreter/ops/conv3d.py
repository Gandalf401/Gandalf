...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if params.__contains__('dilation'):
        ...
        dilation = (1, 1, 1)
    if padding == 'same':
        out_shape = [math.ceil(input_shape[0] / stride[0]),
                     math.ceil(input_shape[1] / stride[1]),
                     math.ceil(input_shape[2] / stride[2]),
                     out_channels]
    else:
        out_shape = [math.floor((input_shape[0] - dilation[0] * (kernel_size[0] - 1) - 1) / stride[0] + 1),
                     ...
                     math.floor((input_shape[2] - dilation[2] * (kernel_size[2] - 1) - 1) / stride[2] + 1),
                     out_channels]
    # 檢查dilation和stride
    if dilation[0] != 1 and stride[0] != 1 \
            or dilation[1] != 1 and stride[1] != 1 \
            or dilation[2] != 1 and stride[2] != 1:
        raise Exception('Dilation and stride of Conv3d cannot be unequal to 1 at the same time.')
    if shape_only:
        return out_shape
    # 分框架实现
    if framework == 'TensorFlow':
        ...
        return eval('tf.keras.layers.Conv3D' + params_str), out_shape
        ...
        if padding == 'valid':
            ...
            return eval('torch.nn.Conv3d' + params_str), out_shape
        else:
            pad_d, pad_h, pad_w = cal_same_padding_3d(input_shape, kernel_size, stride, dilation)
            if pad_d % 2 == 0 and pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(in_channels={0}, out_channels={1}, kernel_size={2}, stride={3}, ' \
                             'padding=({4}, {5}, {6}), bias={7}, dilation={8})'\
                    .format(in_channels, out_channels, kernel_size, stride,
                            pad_d // 2, pad_h // 2, pad_w // 2, bias, dilation)
                return eval('torch.nn.Conv3d' + params_str), out_shape
            else:
                ...
                params_str = '(in_channels={0}, out_channels={1}, kernel_size={2}, stride={3}, ' \
                    ...
                    .format(in_channels, out_channels, kernel_size, stride,
                            pad_d // 2, pad_h // 2, pad_w // 2, bias, dilation)
                return [pad, eval('torch.nn.Conv3d' + params_str)], out_shape
    elif framework == 'Jittor':
        if padding == 'valid':
            params_str = '(in_channels={0}, out_channels={1}, kernel_size={2}, stride={3}, padding=0, bias={4}, ' \
                         'dilation={5})'.format(in_channels, out_channels, kernel_size, stride, bias, dilation)
            return eval('jittor.nn.Conv3d' + params_str), out_shape
        else:
            pad_d, pad_h, pad_w = cal_same_padding_3d(input_shape, kernel_size, stride, dilation)
            if pad_d % 2 == 0 and pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(in_channels={0}, out_channels={1}, kernel_size={2}, stride={3}, ' \
                ...
                params_str = '(in_channels={0}, out_channels={1}, kernel_size={2}, stride={3}, ' \
                    ...
                    .format(in_channels, out_channels, kernel_size, stride,
                            pad_d // 2, pad_h // 2, pad_w // 2, bias, dilation)
                return [pad, eval('jittor.nn.Conv3d' + params_str)], out_shape
    else:
