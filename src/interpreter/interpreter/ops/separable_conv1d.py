...
from src.configuration.support.torch_expansion.separable_conv1d import SeparableConv1d as torch_conv1d
from src.configuration.support.jittor_expansion.separable_conv1d import SeparableConv1d as jittor_conv1d
...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if dilation != 1 and stride != 1:
        raise Exception('Dilation and stride of SeparableConv1d cannot be unequal to 1 at the same time.')
    ...
    if framework == 'TensorFlow':
        ...
        return eval('tf.keras.layers.SeparableConv1D' + params_str), out_shape
        ...
        if padding == 'valid':
            ...
            if pad % 2 == 0:
                params_str = '(in_channels={0}, kernel_size={1}, stride={2}, padding={3}, dilation={4}, ' \
                             'bias={5}, out_channels={6}, depth_multiplier={7})'.format(in_channels, kernel_size,
                                                                                        stride, pad // 2, dilation,
                                                                                        bias, out_channels,
                                                                                        depth_multiplier)
                return eval('torch_conv1d' + params_str), out_shape
            else:
