...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    # 输出维度
    if padding == 'same':
        out_shape = [math.ceil(input_shape[0] / stride[0]), math.ceil(input_shape[1] / stride[1]), input_shape[-1]]
    else:
        out_shape = [math.ceil((input_shape[0] - pool_size[0] + 1) / stride[0]),
                     math.ceil((input_shape[1] - pool_size[1] + 1) / stride[1]), input_shape[-1]]
    if shape_only:
        return out_shape
    if framework == 'TensorFlow':
        return tf.keras.layers.AveragePooling2D(pool_size, stride, padding), out_shape
    elif framework == 'PyTorch':
        if padding == 'valid':
            params_str = '(kernel_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('torch.nn.AvgPool2d' + params_str), out_shape
        else:
            pad_h, pad_w = cal_same_padding(input_shape, pool_size, stride)
            if pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}))' \
                    .format(pool_size, stride, pad_h // 2, pad_w // 2)
                return eval('torch.nn.AvgPool2d' + params_str), out_shape
            else:
                pad = t_same_padding(pad_h, pad_w)
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}))' \
                    .format(pool_size, stride, pad_h // 2, pad_w // 2)
                return [pad, eval('torch.nn.AvgPool2d' + params_str)], out_shape
    elif framework == 'Jittor':
        if padding == 'valid':
            params_str = '(kernel_size={0}, stride={1}, padding=0)'.format(pool_size, stride)
            return eval('jittor.nn.AvgPool2d' + params_str), out_shape
        else:
            pad_h, pad_w = cal_same_padding(input_shape, pool_size, stride)
            if pad_h % 2 == 0 and pad_w % 2 == 0:
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}))' \
                ...
                params_str = '(kernel_size={0}, stride={1}, padding=({2}, {3}))' \
                    .format(pool_size, stride, pad_h // 2, pad_w // 2)
                return [pad, eval('jittor.nn.AvgPool2d' + params_str)], out_shape
    else:
