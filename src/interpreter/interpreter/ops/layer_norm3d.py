...
def get_op_and_shape(input_shape, params, framework, shape_only=False):
    ...
    if framework == 'TensorFlow':
        return tf.keras.layers.LayerNormalization(trainable=affine, epsilon=eps, axis=[1, 2, 3, 4]), input_shape
    elif framework == 'PyTorch':
        normalized_shape = [input_shape[-1], input_shape[1], input_shape[2], input_shape[3]]
